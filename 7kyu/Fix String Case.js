...
function solve(s){
    let array = s.split('')
    let lowerCaseArray = array.filter(char => char === char.toLowerCase())
    let upperCaseArray = array.filter(char => char === char.toUpperCase())
    
      return lowerCaseArray.length >= upperCaseArray.length ? 
