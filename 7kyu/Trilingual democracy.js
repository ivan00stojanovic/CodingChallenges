...
2 Here are the rules for groups of exactly three3 people:4
...
function trilingualDemocracy(group) {
    let inputArray = group.split('')
    let languageArray = ['I', 'D', 'K', 'F']
     if([... new Set (inputArray)].length === 1) return inputArray[0]
    if([... new Set (inputArray)].length === 2){
      let temp = inputArray.pop()
      return inputArray.includes(temp) ? inputArray.filter(char => char != temp)[0] : temp 
    }else if([... new Set (inputArray)].length === 3){
      return languageArray.filter(char => !inputArray.includes(char))[0]
    }
  }
