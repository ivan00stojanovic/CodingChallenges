...
const meeting = (x, need) => {
    ...
    for (const room of x) {
      ...
      if (chairs > occupants) {
        const spareChairs = chairs - occupants;
        const takenChairs = Math.min(spareChairs, need - totalChairs);
        
        totalChairs += takenChairs;
        chairArray.push(takenChairs);
      } else {
        chairArray.push(0);
      }
    ...
    return totalChairs >= need ? chairArray : 'Not enough!';
  };